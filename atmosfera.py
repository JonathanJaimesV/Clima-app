...
def generar_reporte_clima(c):
    ...
    # Extraer valores para evitar f-strings anidados / Extract values to avoid nested f-strings
    ciudad_pais = f"{c['ciudad']}, {c['pais']}"
    temp_c = f"{c['temp']}°C"
    sensacion_c = f"{c['sensacion']}°C"
    humedad_c = f"{c['humedad']}%"
    viento_c = f"{c['viento']} km/h"
    visibilidad_c = f"{c['visibilidad']} km"
    nubosidad_c = f"{c['nubosidad']}%"

    lineas = [
        f"{'─' * (EW + VW + 4)}",
        f"  {'REPORTE DE CLIMA / WEATHER REPORT':^{EW + VW}}",
        f"{'─' * (EW + VW + 4)}",
        f"  {'Ciudad / City':<{EW}} {ciudad_pais:>{VW}}",
        f"  {'Temperatura / Temperature':<{EW}} {temp_c:>{VW}}",
        f"  {'Sensación / Feels Like':<{EW}} {sensacion_c:>{VW}}",
        f"  {'Humedad / Humidity':<{EW}} {humedad_c:>{VW}}",
        f"  {'Viento / Wind':<{EW}} {viento_c:>{VW}}",
        f"  {'Visibilidad / Visibility':<{EW}} {visibilidad_c:>{VW}}",
        f"  {'Nubosidad / Cloudiness':<{EW}} {nubosidad_c:>{VW}}",
        f"  {'Condición / Condition':<{EW}} {c['descripcion']:>{VW}}",
        f"{'─' * (EW + VW + 4)}",
    ]
    return lineas
