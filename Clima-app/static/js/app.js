...
function ocultarTodo() {
  ...
  SECTIONS.forEach(id => {
    ...
    if (el) {
      ...
      el.style.display = '';   // Limpiar cualquier display inline
    }
  });
  // Ocultar el menú principal
  const menu = $id('menu-section');
  if (menu) {
...
function mostrarSeccion(id) {
  ...
  if (el) {
    ...
    el.style.display = 'flex';   // Forzar display correcto
    // flex porque las secciones usan flex-direction: column
  }
...
window.volver = function () {
  ...
  if (menu) {
    ...
    menu.style.display = '';   // Dejar que CSS tome el control
  }
  hideError();
  window.scrollTo({ top: 0, behavior: 'smooth' });
};
